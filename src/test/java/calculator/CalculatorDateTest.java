...
public class CalculatorDateTest extends Calculator {
    ...
    public void goToDate() {
        ...
        assertEquals("Date Calculator should be selected", "DATE CALCULATION", header.getText());
    }
    ...
    public void findDifference() {
        ...
        List<WebElement> elements = driver.findElementsByAccessibilityId("DateDiffAllUnitsResultLabel");
        if (!elements.isEmpty()) {
            System.out.println(elements.get(0).getText());
        }
        String difference = driver.findElement(By.xpath("(//Text)[last()]")).getText();
    ...
    private void selectYear(int year) {
        String currView = driver.findElementByAccessibilityId("HeaderButton").getText();
        String[] years = currView.split("-");
        Range<Integer> yearRange = Range.between(Integer.valueOf(years[0].trim()), Integer.valueOf(years[1].trim()));
    ...
    private void selectMonth(int month) {
        String monthName = WordUtils.capitalize(Month.of(month).name().toLowerCase());
